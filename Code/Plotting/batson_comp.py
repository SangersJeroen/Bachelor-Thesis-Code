...
imspec = np.sum(data.stack[:,700:1400,800:1300], axis=(2,1))

qmap_corr = mreels.batson_correct(data, 4, qmap_slice, imspec)

fig, ax = plt.subplots(2,1, sharex=True)

plt.gcf().set_size_inches(1.6*8, 8)

ax[0].pcolormesh(qaxis_slice, eax, mreels.sigmoid(qmap_slice, (50,150)).T)
ax[0].set_ylim([0,36])
...
ax[1].pcolormesh(qaxis_slice, eax, mreels.sigmoid(qmap_corr, (50,150)).T)
ax[1].set_ylim([0,36])
ax[1].set_ylabel(r"energy loss $\Delta E$ [$eV$]")
ax[1].set_xlabel(r"momentum transfer $q_{\perp}$")
ax[1].vlines(qaxis_slice0.max(), 0, 36, linestyle='--', linewidth=2, color='white')
ax[1].vlines(qaxis_slice1.max(), 0, 36, linestyle='--', linewidth=2, color='white')
...
plt.savefig('qmap_corr_comp.eps', format='eps')
plt.show()
